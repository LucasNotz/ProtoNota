...
public class TelaPrincipal extends JFrame{
	...
	public TelaPrincipal(String mainFolderPath) {
		...
		addNota.setBounds(10,330,100,25);
		...
		list.addMouseListener(new MouseListener() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				new TelaView(mainFolderPath,selectedNote).setVisible(true);
			}
		});
	...
	public static void refresh(String mainFolderPath) {
		...
		if(listOfFiles != null) {
			for(File child: listOfFiles) {
				...
				if(!getExtension(child.toString()).equals(".txt")) {
					continue;
				}
				//add to dlm
